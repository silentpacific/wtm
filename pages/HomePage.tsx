...
const HomePage: React.FC<HomePageProps> = ({ onScanSuccess, onExplanationSuccess }) => {
    ...
    useEffect(() => {
        const checkCanScan = async () => {
            if (!user) {
                setCanScanState(canAnonymousUserScan());
                return;
            }

            try {
                const counters = await getUserCounters(user.id);
                setCanScanState(canUserScan(counters));
            } catch (error) {
    ...
    // Track page visibility changes and user engagement
    useEffect(() => {
    ...
    // Track menu results viewing time
    useEffect(() => {
    ...
    }, [scanResult, restaurantInfo, user]);
    ...
    }, [canScanState, user, userProfile, onScanSuccess]);
...
export default HomePage; className="w-12 h-1 bg-gradient-to-r from-coral-400 to-teal-400 rounded-full"></div>
                  </div>
                </div>
              </div>
